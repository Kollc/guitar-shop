...
import { getCountStartShowGuitars, getFormatedDate, getImagenData, getNumberArrayByCount, getRatingByStars, sortReviewsByDate, getRatingNameValue, addStyleBodyWithOpenModal, addStyleBodyWithCloseModal, calcMinGuitarsPrice, calcMaxGuitarsPrice } from './utils';
...
describe('Util funtions', () => {
  ...
  it('should getMinGuitarsPrice work is succes', () => {
    expect(calcMinGuitarsPrice(testGuitars)).toBe(500);
  });

  it('should getMaxGuitarsPrice work is succes', () => {
    expect(calcMaxGuitarsPrice(testGuitars)).toBe(30000);
  });
});
